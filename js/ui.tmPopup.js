...
$.widget("ui.tmPopup", {
	...
	_create: function() {		
		...
		var self = this,		
			...
			var orgBtn= this.element;
			orgBtn.bind("click.tmPopup", function(){				
				orgBtn.tmPopup("open");
			});
			self._id= titleId;
	...
	close: function(event) {
		var self = this, maxZ, thisZ, options = self.options;
		...
		if (options.hide) {
			self.tmPopup.hide(options.hide, function() {
				self._trigger('close', event);
			});
			self.tmPopupBackground.hide(options.hide);			
		} else {
			self.tmPopup.hide();
			self.tmPopupBackground.hide();
			self._trigger('close', event);
		}		
		if(options.openRefresh){
			self._loaded= false;
		}
	...
	_createButtons: function(buttons) {		
		var self = this,
			...
			uiPopupBtnUl= (self.tmPopupBtnUl = $('<div></div>'))
				.addClass('ui-tmPopup-btn-group')
				.appendTo(uiPopupFooter);
		// if we already have a button set, remove it
		self.tmPopup.find('.ui-tmPopup-footer').remove();
